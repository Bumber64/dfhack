...
DFHACK_EXPORT int Gui::makeAnnouncement(df::announcement_type type, df::announcement_flags flags, df::coord pos, std::string message, int color, bool bright)
{
    ...
    if (flags.bits.PAUSE || flags.bits.RECENTER)
        pauseRecenter((flags.bits.RECENTER ? pos : df::coord()), flags.bits.PAUSE); // Does nothing outside dwarf mode
    
    if (flags.bits.DO_MEGA && (*gamemode != game_mode::ADVENTURE || world->units.active.empty() || world->units.active[0]->counters.unconscious <= 0))
        showPopupAnnouncement(message, color, bright);

    vector<string> results;
    parseReportString(results, message, (init->display.grid_x - 7));
    if (results.empty())
    {
    ...
    // Not a repeat, write the unparsed message to gamelog.txt
...
bool Gui::revealInDwarfmodeMap(df::coord pos, bool center)
{
    ...
    recenterViewscreen(pos, (center ? report_zoom_type::Unit : report_zoom_type::Item));
